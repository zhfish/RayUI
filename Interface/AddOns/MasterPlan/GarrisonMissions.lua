...
local sortIndicator = CreateFrame("Button", nil, GarrisonMissionFrameMissions, nil) do
	...
	lab:SetPoint("LEFT", 15, 0) lab:SetText("任务排序:")
	...
	EV.RegisterEvent("MP_SETTINGS_CHANGED", function(ev, s)
		if s == nil or s == "availableMissionSort" then
			sortIndicator:SetText(MasterPlan:GetMissionOrder() == "threats" and "更高成功率" or "更高等级")
		end
	end)
...
do -- Active Missions tab
	...
	tab.Text:SetText("已激活任务")
	GarrisonMissionFrameTab1:SetText("可用任务")
	...
	local function updateMissionCounts(useCachedData)
		...
		tab:SetFormattedText("已激活任务(%d)", #GarrisonMissionFrameMissions.inProgressMissions)
		GarrisonMissionFrameTab1:SetFormattedText("可用任务(%d)", #GarrisonMissionFrameMissions.availableMissions)
	...
	for i=1, #buttons do
		...
		if btn:IsShown() then
			...
			if not follower.isCollected or follower.status == GARRISON_FOLLOWER_INACTIVE or follower.levelXP == 0 then
				...
				st:SetFormattedText("%s经验", BreakUpLargeNumbers(follower.levelXP - follower.xp))
			end
		end
	end
end)
...
local UpdateCompletedMissionList do -- Rearranged completion dialog
	...
	lootContainer.noBagSlots:SetText("|cffff2020你的背包空间不足.|r|n任务奖励将以邮件发送给你.")
	lootContainer:Hide()
	...
	local function UpdateCompleteButton(isBusy, isFinished, hasLoot)
		...
		bf.CompleteAll:SetText(isFinished and ((lootContainer:IsShown() or not hasLoot) and "完成" or "查看奖励") or "完成任务")
	...
	local function updateMissionStatusList(completeMissions)
		for i=1,#completeMissions do
			...
			mb.status:SetText(mi.failed and "|cffff2020失败|r" or (mi.state >= 0 or mi.succeeded) and "|cff20ff20成功|r" or (NORMAL_FONT_COLOR_CODE .. "已完成" .. (sc and " (" .. floor(sc)  .. "%)" or "")))
			if sc and mm then
