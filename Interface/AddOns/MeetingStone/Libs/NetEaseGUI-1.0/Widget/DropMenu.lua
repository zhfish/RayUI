...
function DropMenu:OnItemEnter(button, data)
    ...
    if (data.tooltipTitle or data.tooltipText) and (data.tooltipWhileDisabled or button:IsEnabled()) then
        if data.tooltipOnButton then
            GameTooltip:SetOwner(button, 'ANCHOR_RIGHT')
        else
            GameTooltip_SetDefaultAnchor(GameTooltip, button)
        end
        GameTooltip:AddLine(data.tooltipTitle, 1, 1, 1)
        GameTooltip:AddLine(data.tooltipText, nil, nil, nil, true)
        GameTooltip:Show()
    end
end
